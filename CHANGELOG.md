...
- Explicit support for committing and pushing changelog updates back to the repository when run with a token that has `contents: write`. The action stages, commits (with the provided or default commit message), and pushes the updated `CHANGELOG.md`.
- Added the `commit-message` input to allow consumers to provide a custom commit message for the changelog update (defaults to `chore: update CHANGELOG.md with dependency changes`).
